...
//
//	@param filename : C string that holds the file name of the xml project file
//
/////////////////////////////////////////////////////////////////////////////////////////////////////////
...
void Scene::readScene(const char* filename)
{
    ...
    if(useBoundingVolumes)
    {
        this->loadBoundingVolumes(geometry);
    }
    else
    {
...
//
//	@param settings : A pointer to a TinyXmlElement representing the <settings> block.
//
/////////////////////////////////////////////////////////////////////////////////////////////////////////
...
bool Scene::loadSettings( tinyxml2::XMLElement *settings )
{
	...
	for( ; child; child=child->NextSiblingElement())
	{
        ...
        else if(strcmp(typeStr, "useBoundingVolumes") == 0)
        {
            useBoundingVolumes = child->BoolAttribute(kValue);
        }
	}
...
bool Scene::loadBoundingVolumes(tinyxml2::XMLElement *geometry)
{
    tinyxml2::XMLElement *bv = geometry->FirstChild()->ToElement();
    
    for ( ; bv; bv=bv->NextSiblingElement() )
    {
        ...
        else if(strcmp(prType, "annulus" ) == 0)
        {
            ...
            bv->FirstChildElement( "smallRadius" )->QueryFloatAttribute( "value", &smallRadius );
            bv->FirstChildElement( "largeRadius" )->QueryFloatAttribute( "value", &largeRadius );
        ...
        if(object->type != none)
        {
            this->initBV( bv, *object);
            this->boundingVolumes.push_back(*object);
        }
    }
    return true;
}
...
//			   object
//
//	@param xmlElement   : A pointer to a TiXmlElement representing the <primitive> block.
//	@param primative    : A reference to a ScnObject to which the parsed attributes will be applied.
//
/////////////////////////////////////////////////////////////////////////////////////////////////////////
...
bool Scene::initBV( tinyxml2::XMLElement *xmlElement, BoundingVolume &object )
{
    if(!(xmlElement->FirstChild()))
    {
        return true;
    ...
    for( ; child; child=child->NextSiblingElement())
    {
        ...
        //Should be done in the order of scale, rotate, translate
        if(strcmp(typeStr, "translation") == 0)
        {
            float xTrans = child->FloatAttribute("x");
            float yTrans = child->FloatAttribute( "y");
            float zTrans = child->FloatAttribute( "z");
        ...
        else if(strcmp(typeStr, "children") == 0)
        {
            object.objStartIndex = (int)this->primitives.size();
            this->loadGeometry(child);
            object.objEndIndex = (int)this->primitives.size();
        }
