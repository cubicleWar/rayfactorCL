...
//inline float discHit(local struct Primitive *p, const Ray *wsRay) {
inline float discHit(global const Primitive *p, const Ray *wsRay) {
    Ray osRay;
    
    xfrmRay(&p->invm[0], &osRay, wsRay);
...
//inline void annulusRay(const float4 *rands, Ray *ray, local struct Primitive *p) {
...
inline float annulusHit(global const Primitive *p, const Ray *wsRay) {
    Ray osRay;
    ...
    if(exp >= p->util1 && exp <= p->util2 && th > EPS) {
        return th;
    } else {
        return INFINITY;
    }
...
inline void cylinderRay(const float4 *rands, Ray *ray, global const struct Primitive *p) {
    ...
    // Normal ius based on the os surface points
...
inline float cylinderHit(global const Primitive *p, const Ray *wsRay) {
    Ray osRay;
...
inline float frustumHit(global const Primitive *p, const Ray *wsRay) {
    Ray osRay;
    
    xfrmRay(&p->invm[0], &osRay, wsRay);
    
    const float fDir = (p->util1 - 1.0f)*osRay.c.z;
    const float fStart = (p->util1 - 1.0f)*osRay.s.z + 1.0f;
...
inline float rectangleHit(global const struct Primitive *p, const Ray *wsRay) {
    Ray osRay;
...
inline float sphereHit(global const struct Primitive *p, const Ray *wsRay) {
    Ray osRay;
...
// Version using local memory to store the hit data first
__kernel void runGPU(int fromIndex, const int n_objects, global const Primitive *objects, __local int *x_res, __global int *hits) {
    ...
    int lid = get_local_id(0);  // Maybe should be a size_t?? thats what get_local_id returns
    ...
    for(int v = 0; v < 4; v++) {
        ...
        for(int w = 0; w < n_objects; w++) {
            switch(objects[w].type) {
                case rectangle  : t = rectangleHit(&objects[w], &ray); break;
                case disc       : t = discHit(&objects[w], &ray); break;
                case annulus    : t = annulusHit(&objects[w], &ray); break;
                case cylinder   : t = cylinderHit(&objects[w], &ray); break;
                case sphere     : t = sphereHit(&objects[w], &ray); break;
                case frustum    : t = frustumHit(&objects[w], &ray); break;
                default: break;
            }
    ...
    // This reduction section takes around 4.2% of the run time on the sample case
    
     n = lid;
     while(n < n_objects) {
         atomic_add(&hits[fromIndex*n_objects + n], x_res[n]);
         n += lsize;
     }
}
