...
inline void transformPoint(global const float *m, const Vector *osPoint, Vector *wsPoint) {
    //wsPoint->x = m[0]*osPoint->x + m[1]*osPoint->y + m[2]*osPoint->z + m[3];
    //wsPoint->y = m[4]*osPoint->x + m[5]*osPoint->y + m[6]*osPoint->z + m[7];
    //wsPoint->z = m[8]*osPoint->x + m[9]*osPoint->y + m[10]*osPoint->z + m[11];
...
__kernel void runGPU(int fromIndex, const int n_objects, global const Primitive *objects, __local int *x_res, __global int *hits) {
    ...
    for(int v = 0; v < 4; v++) {
        ...
        if(objectNo != -1) atomic_inc(&hits[fromIndex*n_objects + objectNo]);
    } // end for sub rays
}